...
export default function MapBase({className, list, mapBaseComponent, children}) {
  ...
  return (
    <div className={classNames("map-base", className, styles.mapBase)}>
      <MapBaseBg zoom={libraryMapZoom} setLibraryMapZoom={setLibraryMapZoom}>
        {mapBaseComponent ? mapBaseComponent() : null}
        {items ? <div className={classNames("map-base__points",styles.mapBase__points)}>{items}</div> : null}
        {/*{children}*/}
      </MapBaseBg>
      <Zoom className={classNames("map-base__zoom",styles.mapBase__zoom)}
