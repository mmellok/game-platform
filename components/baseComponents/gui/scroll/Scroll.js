...
const Scroll = React.forwardRef(
  function Scroll({mod, update, children, onScroll, resetRef, showShadow = false, ...settings}) {
    ...
    return (
      <div className={classNames("scroll-wrapper", scroll.scrollWrapper,{
        ...
        {showShadow && <div ref={shadowTop}
                            className={classNames(scroll.scroll__shadow, scroll.scroll__shadow_top)}/>}
        {showShadow && <div ref={shadowBottom}
                            className={classNames(scroll.scroll__shadow, scroll.scroll__shadow_bottom)}/>}
      </div>
    );
  });
