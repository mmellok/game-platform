...
export default class Builder {
  ...
  createSelector(name, selector) {
    const memoized = createSelector(
      ...
      ([state, ...args]) => selector(state, ...args)
    );

    this.selectors[camelCase(`use-${name}`)] = function useSomeSelector(...args) {
      return useSelector((state) => memoized(state, ...args));
    };
    return this;
  }
    ...
    const slice = createSlice({
      ...
      extraReducers: builder => {
        Object
          ...
          .forEach(([actionName, reducer]) => builder.addCase(actionName, (...args) => this.checkCanceled(reducer, ...args)));
  ...
  checkCanceled(reducer, state, action) {
